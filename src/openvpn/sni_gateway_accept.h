 ...
 * when --sni-gateway-server auto is active (both SF_SNI_GW_SERVER_PASSTHROUGH and
 * SF_SNI_GW_SERVER_HTTP are set on the same socket).  Decides, from the first
 ...
 * the existing lazy SF_SNI_GW_SERVER_PASSTHROUGH peek in stream_buf_added() and to
